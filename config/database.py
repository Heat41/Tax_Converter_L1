...
def init_database(db_path: Optional[Path | str] = None) -> None:
    """Inisialisasi tabel SQLite dari file DDL utama/tambahan dan seeding data master."""
    conn = get_db_connection(db_path)

    # 1. Jalankan skema utama.
    with open(SCHEMA_FILE_PATH, "r", encoding="utf-8") as f:
        conn.executescript(f.read())

    # 2. Jalankan skema tambahan secara idempotent.
    for schema_path in EXTRA_SCHEMA_FILES:
        if not schema_path.exists():
            continue
        with open(schema_path, "r", encoding="utf-8") as f:
            conn.executescript(f.read())

    # 3. Seed Master Reference Code (35 Mapping E-Form <-> Coretax)
