...
from core.legacy_1770_lampiran_i import Legacy1770LampiranIService as BaseLegacy1770LampiranIService
...
class Legacy1770LampiranIService(BaseLegacy1770LampiranIService):
    ...
    # Koordinat diambil dari garis/kotak master bersih halaman 2 (origin kiri-atas).
    ...
    # Pusat 16 kotak NPWP pada master halaman 2. Kelompok terakhir tidak
    # seluruhnya berjarak sama, sehingga dipakai titik per digit agar presisi.
    ...
    @classmethod
    def _draw_year_at(cls, canvas, rect: Rect, year: int, width: float, height: float) -> None:
    ...
    @classmethod
    def _draw_page1_npwp(cls, canvas, npwp: str, width: float, height: float) -> None:
        ...
        # Baseline dikalibrasi dari tinggi kotak NPWP halaman 2, bukan geometric
        # center karakter, supaya digit tidak menempel pada border atas/bawah.
        box_y0_top = 121.22
        box_y1_top = 136.46
    ...
    def _draw_page1_header(self, canvas, document: Legacy1770Document, width: float, height: float) -> None:
        self._draw_year_at(canvas, self.PAGE1_YEAR_RECT, document.tahun_pajak, width, height)
        ...
        self._draw_left(
            canvas,
            self.PAGE1_NAME_RECT,
            str(document.nama_wp or "").upper(),
            width,
            height,
            size=7.0,
        )
